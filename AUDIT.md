...
## 8. Hardening pass — 2026-09-19

- **Red CI test fixed:** the multi-league Dixon-Coles backtest (`ballotBacktest.test.ts`) timed out at the 180s limit on slower machines. The per-row model sweep is now memoized per unique odds tuple (bookmaker averages repeat heavily across 7k+ rows) and the backtest-family timeouts were raised to 420s. Isolated run: 19.5s, passing. `footballMarketsBacktest` / `momentumBacktest` timeouts raised identically.
- **PII gate (`users:getProfile`):** the query was public and returned full `userProfiles` rows (mobile, DOB, state of residence) for any email. It now requires the caller's Convex identity email to match the requested email (or super admin) and strips the sensitive contact fields from the response.
