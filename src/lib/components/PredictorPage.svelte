<script lang="ts">
  ...
  import { ArrowLeft, ArrowDown, ArrowUp, Inbox, Clock3, Check, ChevronLeft, ChevronDown, Wand2, Activity, Search, X } from '@lucide/svelte';
  ...
  // Fixed 7-day window (today → +6). Fixtures appear automatically as the
  // cron pipeline caches them — no manual date picking anywhere in the UI.
  const fromDay = dayKeyFor(0);
  const toDay = dayKeyFor(6);
  const timeBand: TimeBand = 'all';
...
$effect(() => {
  ...
  onMount(() => {
    // Scores, fixtures and verdicts are FULLY cron-driven: live scores sync
    // every 15 minutes, fixtures/verdicts refresh three times daily per sport.
    // No manual triggers exist in the UI — a page visit costs zero writes.
      ...
      {#if phase === 'select'}
        ...
        {#if windowDays.length > 0}
          {#each windowDays as dk (dk)}
            ...
            <section class="day-group">
              ...
              {#if dayHasData(dk) && dayMatches.length === 0}
                  ...
                  <span>
                    {gameTab !== 'upcoming'
                      ...
                      : dayHasData(dk)
                        ...
                        : 'No fixtures on this day yet — the agent team caches new fixtures on every scheduled cycle.'}
                  </span>
                </div>
              {/if}
            </section>
        ...
        {:else if !searchActive}
          <div class="empty-state">
            ...
            <p>No scheduled matches cached in the {fromDay}…{toDay} window yet.</p>
            <p class="hint">
